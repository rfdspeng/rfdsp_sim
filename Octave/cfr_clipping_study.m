...
% It doesn't matter how many iterations you do, you cannot recover EVM. This makes sense, since clipping and filtering
% only allows you to suppress the out-of-band noise. Over iterations, the inband noise builds up but cannot be removed.
% Increasing the number of iterations does improve your ACLR, however.
...
for idx = 1:niter
  ...
  cfg_evm.en_plot = en_plot && idx == niter; %cfg_evm.title = "HC";
...
if en_plot
  figure; hold on;
  semilogy(bins,ccdf);
  semilogy(bins,ccdfs(niter,:));
endif

% Plot AMAM
if en_plot
  figure; hold on;
  plot(abs(x),abs(x_hcfilts(niter,:)),'.');
...
if en_plot
  ...
  p_hcfilt = calculate_psd(x_hcfilts(niter,:),fs,rbw); p_hcfilt = scale_psd(p_hcfilt,f,bw,scs,start_sc,num_sc);
  figure; hold on;
  plot(f,10*log10(p),'linewidth',5);
  plot(f,10*log10(p_hcfilt),'linewidth',1);
  title("CFR PSDs"); xlabel("Frequency (MHz)"); ylabel("PSD (dB)");
  legend("Mod","CFR",'location','south');
  xlim([-fs/2 fs/2]);
  grid on; set(gca,"fontsize",40);
endif
